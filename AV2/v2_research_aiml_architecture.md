...
| :--- | :--- | :--- |
| **No Digital Twin** | HIL step fails to initialize | Bypass to abstract BMC verification $\rightarrow$ Mandate strict Human-in-the-Loop (HITL) manual sign-off. |
| **Infinite Regen Loop** | Verification fails > $N$ times | Agent halts generation $\rightarrow$ Packages trace logs, failure reasons, and BMC counter-examples $\rightarrow$ Escalates to human engineer $\rightarrow$ Reverts to safe standby state. |
...
| **RAG Retrieval Failure** | Low confidence score from Vector DB | Agent stops generating assumptions $\rightarrow$ Queries human for specific documentation $\rightarrow$ Learns from new input. |
