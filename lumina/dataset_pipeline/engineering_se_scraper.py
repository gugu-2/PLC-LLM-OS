...
def fetch_answers(answer_ids):
    ...
    ans_url = f"https://api.stackexchange.com/2.3/answers/{ids_str}?order=desc&sort=activity&site=engineering&filter=withbody"
    resp = requests.get(ans_url)
    if resp.status_code == 200:
        return resp.json().get("items", [])
    return []

def scrape_stackoverflow_plc(tags, output_file="data/engineering_se_plc_qa.jsonl"):
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    
    print(f"Scraping Engineering StackExchange for tags: {tags}...")
...
if __name__ == "__main__":
    ...
    tags_to_scrape = [
        ...
        "scada", "pid", "ladder-logic", "industrial-automation"
    ]
